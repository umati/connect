...
def find_value_in_stream(
    ...
    if not component_streams:
        raise MissingComponentStreamError(
            f"No ComponentStream found with component='{component}' and name='{component_name}'."
        )

    print(
        f"[UPDATING] Found {len(component_streams)} ComponentStream(s) for component='{component}' and name='{component_name}'."
    )
    ...
    for cs in component_streams:
        for elem in cs.iterfind(f".//m:{element_name}", ns):
            if subtype == "None" or elem.attrib.get("subType") == subtype:
                ...
                print(
                    f"\033[92m[SUCCESS] Updated {element_name} with value '{new_value}'\033[0m"
                )
                return
    raise MissingElementError(
        f"Element '{element_name}' with optional subType='{subtype}' not found."
    )


def add_element_to_stream(
    xml_object: XmlState,
    component: str,
    component_name: str,
    element_name: str,
    ...
    if subcomponent_element is None:
        raise MissingSubcomponentError(
            f"No subcomponent '{subcomponent}' found in ComponentStream for component='{component}' and name='{component_name}'."
        )
    ...
    print(
        f"\033[92m[SUCCESS] Added new element '{element_name}' with subType='{subtype}' and value='{value}' to ComponentStream.\033[0m"
    )


def add_component_stream(
    xml_object: XmlState,
    devicestream_name: str,
    component: str,
    component_name: str,
    ns: dict,
) -> ET.ElementTree:
    ...
    if device_stream is None:
        raise DeviceStreamError(
            f"No DeviceStream found with name='{devicestream_name}'."
        )
    ...
    new_component_stream = ET.Element(
        comp_stream_tag, {"component": component, "name": component_name}
    )
    ...
    print(
        f"\033[92m[SUCCESS] Added new ComponentStream for component='{component}' and name='{component_name}'.\033[0m"
    )
